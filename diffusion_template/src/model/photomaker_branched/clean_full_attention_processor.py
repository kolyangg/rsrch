"""Audited hard-v1 attention processor used by clean_full configs."""
...
from .clean_full_attention_extensions import RecentAttentionExtensionsMixin
from .clean_full_identity_conditioning import (
...
class BranchedAttnProcessor(RecentAttentionExtensionsMixin, nn.Module):
    """
    ...
    def _call_hardcase(self, attn, hidden_states, temb) -> torch.Tensor:
        ...
        if mode == "temporal_frequency":
            ...
            if self.semantic_window_gate_enabled:
                window_scale, semantic_window_metrics = self._semantic_window_scale(
                    target, reference, progress
                )
                high_scale = high_scale * window_scale
                extension_metrics.update(**semantic_window_metrics)
