...
def two_branch_predict(
    ...
    # Extract merged result (mask-blended halves)
    ### 24 Nov: fixing oneid training issues
    noise_pred_gen = noise_pred[:batch_size]
    noise_pred_face = noise_pred[batch_size:]

    USE_SOFT_BLENDING = True
    
    if USE_SOFT_BLENDING and mask4 is not None and mask4.shape[-2:] == noise_pred_gen.shape[-2:]:
    ...
    noise_bg = noise_pred_merged * (1 - mask_4ch) if mask4 is not None else noise_pred_merged
    noise_face = noise_pred_merged * mask_4ch if mask4 is not None else noise_pred_merged
    ### 24 Nov: fixing oneid training issues
