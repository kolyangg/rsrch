...
branched_new2.py - Branched attention implementation that installs trainable
attention processors (from attn_processor2) and tolerates pre-installation
before masks are known.
### Modified to make attn_processor trainable in branched version ###
"""
...
def patch_unet_attention_processors(
    ...
    """
    Patch UNet with branched attention processors (self and cross-attn).
    Safe to call early with mask=None to pre-install modules with parameters.
    """
    # ### Modified to make attn_processor trainable in branched version ###
...
def two_branch_predict(
    ...
    # ### Modified to make attn_processor trainable in branched version ###
    # Install/refresh processors. Allow None masks if pre-installing.
    ...
    if (face_embed_strategy or "face") in {"id"}:
        ...
        if class_tokens_mask is not None:
            ...
            m = m.unsqueeze(-1).to(dtype=d)
            id_scale = torch.tensor(getattr(pipeline, "id_token_scale", 2.5), device=dev, dtype=d)
    ...
    timestep_cond_doubled = torch.cat([timestep_cond, timestep_cond], dim=0) if timestep_cond is not None else None
    ...
    noise_pred_merged = noise_pred[:B2]
