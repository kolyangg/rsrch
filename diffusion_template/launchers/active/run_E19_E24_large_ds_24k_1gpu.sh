...
: "${RUN_NAME:?Set the unique E19-E24 run name}"
: "${CONFIG_NAME:?Set the matching E19-E24 config name}"
...
if [[ "$#" -ne 0 ]]; then
  echo "E19-E24 launchers do not accept ad-hoc Hydra overrides." >&2
  exit 2
fi
case "${CONFIG_NAME}" in
  ...
  E24_large_ds_joint_shadow_sa128_alternating_24k)
    ;;
  *)
    echo "Unapproved E19-E24 CONFIG_NAME: ${CONFIG_NAME}" >&2
    exit 2
    ;;
esac

python tools/validate_E19_E24_config.py \
  --config-name "${CONFIG_NAME}" \
  --run-name "${RUN_NAME}" \
  --experiment-spec "${EXPERIMENT_SPEC_PATH}"

if [[ "${CONFIG_NAME}" == E22_large_ds_joint_shadow_sa128_arcfaceaux_24k ]]; then
  : "${ARCFACE_ONNX_PATH:?Set the hash-pinned ArcFace ONNX path for E22}"
  python tools/verify_arcface_identity_aux.py \
    --model "${ARCFACE_ONNX_PATH}" \
