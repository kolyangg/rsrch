...
CONTINUATION_BASE_CHECK_MODE="${CONTINUATION_BASE_CHECK_MODE:-full}"
# Historical 4k checkpoints use 500-step epoch numbers; do not reinterpret
# those immutable endpoints under the new 2,000-step default.
...
case "${CONTINUATION_BASE_CHECK_MODE}" in
  full|portable_recovery) ;;
  *)
    echo "CONTINUATION_BASE_CHECK_MODE must be full or portable_recovery" >&2
    exit 2
    ;;
esac
...
if [[ "${CONTINUATION_BASE_CHECK_MODE}" == "full" ]]; then
  ...
  # 27 Jul 2026 - Cross-machine recovery retains immutable Comet/checkpoint
  # identities but cannot reproduce machine-absolute paths embedded in the
  # completed base record. The source/base record check above and the
  # continuation prefix/pixel checks below remain mandatory.
  echo "PORTABLE_RECOVERY_BASE_RECORD_VERIFIED source=${SOURCE_RUN} eval=${EVAL_RUN}"
fi
