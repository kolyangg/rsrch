#!/usr/bin/env bash
# CL27-CL29: one-delta CL23 follow-ups on the fixed full96 contract.
set -euo pipefail
...
: "${RUN_NAME:?Set the unique CL27-CL29 run name}"
...
if [[ "$#" -ne 0 ]]; then
  echo "CL27-CL29 launchers reject ad-hoc Hydra overrides." >&2
  exit 2
fi
case "${CONFIG_NAME}" in
...
python tools/validate_CL27_CL29_config.py \
