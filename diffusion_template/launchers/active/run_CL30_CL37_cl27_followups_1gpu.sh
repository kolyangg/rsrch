#!/usr/bin/env bash
# CL30-CL37: isolated CL27 successors on the fixed manual_val96 contract.
set -euo pipefail
...
: "${RUN_NAME:?Set the unique CL30-CL37 run name}"
...
if [[ "$#" -ne 0 ]]; then
  echo "CL30-CL37 launchers reject ad-hoc Hydra overrides." >&2
  exit 2
fi
case "${CONFIG_NAME}" in
  CL30_cosmic_positive_lowband_sameid_24k|\
  CL31_cosmic_attention_ownership_alignment_24k|\
  CL32_cosmic_contact_frequency_surface_24k|\
  CL33_cosmic_visibility_balanced_reconstruction_24k|\
  CL34_cosmic_shared_frequency_calibration_24k|\
  CL35_cosmic_attention_gated_patch_identity_24k|\
  CL36_cosmic_ba_arcface_hinge_4k|\
  CL37_cosmic_smallface_roi_teacher_distill_24k|\
  CL31_cosmic_attention_ownership_alignment_oneval_smoke|\
  CL35_cosmic_attention_gated_patch_identity_oneval_smoke|\
  CL36_cosmic_ba_arcface_hinge_oneval_smoke) ;;
  *) echo "Unapproved CL30-CL37 config: ${CONFIG_NAME}" >&2; exit 2 ;;
esac

test -s "${COSMIC_LARGE_MANIFEST}"
test -d "${COSMIC_LARGE_ROOT}"
if [[ "${CONFIG_NAME}" == CL36_* ]]; then
  : "${CL36_SOURCE_CHECKPOINT:?Set immutable CL27-r3 16k weights}"
  test -s "${CL36_SOURCE_CHECKPOINT}"
  test "$(sha256sum "${CL36_SOURCE_CHECKPOINT}" | cut -d' ' -f1)" = \
    "ba57ad1351f7e4a17ec6a5cf6ef53c34966d673ded4cf58853a0914fbec07591"
fi

python tools/validate_CL30_CL37_config.py \
...
if [[ "${CONFIG_NAME}" == CL36_* ]]; then EXPECTED_STEPS="0,2000,4000"; fi
