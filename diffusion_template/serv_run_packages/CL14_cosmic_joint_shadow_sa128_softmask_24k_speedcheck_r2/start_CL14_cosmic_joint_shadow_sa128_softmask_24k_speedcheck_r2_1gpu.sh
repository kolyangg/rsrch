...
RUN_ID="CL14_cosmic_joint_shadow_sa128_softmask_24k_speedcheck_r2"
CONDA_ENV="${OWNER_ROOT}/conda_env/photomaker_NS"
RUNTIME_ROOT="${OWNER_ROOT}/runtime_sources_cl14_speedcheck_v2/${RUN_ID}"
PROJECT_ROOT="${RUNTIME_ROOT}/diffusion_template"
SOURCE_MANIFEST="${RUNTIME_ROOT}/source_manifest.json"
DATASET_TARGET="${OWNER_ROOT}/rsrch_test/dataset_full"
DATASET_LINK="${RUNTIME_ROOT}/dataset_full"
...
test -d "${DATASET_TARGET}"
if [[ -L "${DATASET_LINK}" ]]; then
  test "$(readlink -f "${DATASET_LINK}")" = "$(readlink -f "${DATASET_TARGET}")"
elif [[ -e "${DATASET_LINK}" ]]; then
  echo "Unexpected non-symlink dataset path: ${DATASET_LINK}" >&2
  exit 76
else
  ln -s "${DATASET_TARGET}" "${DATASET_LINK}"
fi
...
exec bash launchers/active/run_CL14_speedcheck_1gpu.sh
