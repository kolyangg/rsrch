...
RUN_ID="rhca_cosmic_full_crop20_posefirst_trainpar0_valpar100_20k"
...
record_path = Path(
    "saved/rhca_cosmic_full_crop20_posefirst_4k_fast_r3_full96_par100/"
    "comet_experiment.json"
)
record = json.loads(record_path.read_text())
if record["comet"]["experiment_key"] != "90c8297973a7456496200e9f8c042755":
    raise RuntimeError("Ratio-1.0 gate has the wrong Comet experiment key")
result = record.get("validation_result") or {}
verification = result.get("comet_verification") or {}
metrics = verification.get("metric_values") or {}
if result.get("image_count") != 96 or not verification.get("verified"):
    raise RuntimeError("Ratio-1.0 gate is not a finalized 96-image result")
if result.get("intervention_label") != "pose_adapt_ratio=1.0":
    raise RuntimeError("Ratio-1.0 intervention provenance is missing")
if float(metrics.get("manual_val/id_sim", 0.0)) < 0.44:
    raise RuntimeError("Ratio-1.0 identity metric no longer satisfies the gate")
print("POSE_ADAPT_LONG_RUN_GATE_OK", record_path)
PY
...
export EXPERIMENT_ARM="crop20_posefirst_trainpar0_valpar100_20k"
