...
# 10 Aug 2026 - Fixed-checkpoint causal validation only. The immutable CL9
# runtime supplies model code/config; package overlays supply auditable sidecars.
TASK_OWNER_ROOT="/mnt/virtual_ai0001053-01309_SR006-nfs1/nasilaev"
TASK_PACKAGE="/mnt/virtual_ai0001053-01309_SR006-nfs1/nasilaev/rsrch_test/diffusion_template/serv_run_packages/cl9v_marion_occlusion_20260810_r1"
TASK_ROOT="${TASK_OWNER_ROOT}/analysis_sidecars/cl9v_marion_occlusion_20260810_r1"
...
TASK_VISIBILITY="${TASK_PACKAGE}/occlusion_visibility_plan.json"
...
sha256sum "${TASK_CHECKPOINT}" "${TASK_HISTORICAL}" "${TASK_MARION}" \
  "${TASK_LEGACY_EMBEDS}" "${TASK_SUBJECT_EMBEDS}" "${TASK_BBOX}" \
  "${TASK_VISIBILITY}" "${TASK_EVALUATOR}" "${TASK_WRAPPER}" \
  "${TASK_PREPARE}" "${TASK_VERIFY}" "${TASK_SCORE}" \
...
TASK_STAGE="prepare_marion_samefile_transforms"
...
run_fixed() {
  ...
  local task_output="$2"
  shift 2
  TASK_STAGE="generate:${task_variant}"
  printf '%s\n' "${TASK_STAGE}" | tee "${TASK_ROOT}/CURRENT_STAGE"
  cd "${TASK_PROJECT}"
...
run_fixed baseline "${TASK_OUTPUT}/marion_historical_replay" \
  --sample-indices 84,85,86,87,88,89,90,91,92,93,94,95
TASK_STAGE="gate:marion_historical_replay"
"${TASK_ENV}/bin/python" "${TASK_VERIFY}" \
  --replay-dir "${TASK_OUTPUT}/marion_historical_replay" \
  --historical-manifest "${TASK_HISTORICAL}" \
  --expect-count 12 \
  --output "${TASK_OUTPUT}/marion_historical_replay_verification.json"

run_fixed marion_roll "${TASK_OUTPUT}/CL9V_marion_samefile_roll_24k_r1" \
  --sample-indices 84,85,86,87,88,89,90,91,92,93,94,95 \
  --reference-transform-manifest "${TASK_TRANSFORMS}"
run_fixed marion_similarity "${TASK_OUTPUT}/CL9V_marion_fivepoint_similarity_24k_r1" \
  --sample-indices 84,85,86,87,88,89,90,91,92,93,94,95 \
  --reference-transform-manifest "${TASK_TRANSFORMS}"
run_fixed occlusion_ownership "${TASK_OUTPUT}/CL9V_occluder_ownership_24k_r1" \
  --target-visibility-plan "${TASK_VISIBILITY}"

TASK_STAGE="gate:occlusion_80_sentinels"
"${TASK_ENV}/bin/python" "${TASK_VERIFY}" \
  --replay-dir "${TASK_OUTPUT}/CL9V_occluder_ownership_24k_r1" \
  --historical-manifest "${TASK_HISTORICAL}" \
  --skip-indices 2,7,14,19,26,31,38,43,50,55,62,67,74,79,86,91 \
  --expect-count 80 \
  --output "${TASK_OUTPUT}/occlusion_sentinel_verification.json"
...
score_run marion_historical_replay "${TASK_OUTPUT}/marion_historical_replay"
score_run marion_roll "${TASK_OUTPUT}/CL9V_marion_samefile_roll_24k_r1"
score_run marion_similarity "${TASK_OUTPUT}/CL9V_marion_fivepoint_similarity_24k_r1"
score_run occlusion_ownership "${TASK_OUTPUT}/CL9V_occluder_ownership_24k_r1"
