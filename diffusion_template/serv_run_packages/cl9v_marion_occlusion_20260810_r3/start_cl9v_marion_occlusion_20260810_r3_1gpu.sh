...
TASK_PACKAGE="/mnt/virtual_ai0001053-01309_SR006-nfs1/nasilaev/rsrch_test/diffusion_template/serv_run_packages/cl9v_marion_occlusion_20260810_r3"
TASK_ROOT="${TASK_OWNER_ROOT}/analysis_sidecars/cl9v_marion_occlusion_20260810_r3"
...
run_fixed marion_roll "${TASK_OUTPUT}/CL9V_marion_samefile_roll_24k_r3" \
  --reference-transform-manifest "${TASK_TRANSFORMS}"
TASK_STAGE="gate:marion_roll_84_sentinels"
"${TASK_ENV}/bin/python" "${TASK_VERIFY}" \
  --replay-dir "${TASK_OUTPUT}/CL9V_marion_samefile_roll_24k_r3" \
  --historical-manifest "${TASK_HISTORICAL}" \
  --skip-indices 84,85,86,87,88,89,90,91,92,93,94,95 \
  --expect-count 84 \
  --output "${TASK_OUTPUT}/marion_roll_sentinel_verification.json"

run_fixed marion_similarity "${TASK_OUTPUT}/CL9V_marion_fivepoint_similarity_24k_r3" \
  --reference-transform-manifest "${TASK_TRANSFORMS}"
TASK_STAGE="gate:marion_similarity_84_sentinels"
"${TASK_ENV}/bin/python" "${TASK_VERIFY}" \
  --replay-dir "${TASK_OUTPUT}/CL9V_marion_fivepoint_similarity_24k_r3" \
  --historical-manifest "${TASK_HISTORICAL}" \
  --skip-indices 84,85,86,87,88,89,90,91,92,93,94,95 \
  --expect-count 84 \
  --output "${TASK_OUTPUT}/marion_similarity_sentinel_verification.json"

run_fixed occlusion_ownership "${TASK_OUTPUT}/CL9V_occluder_ownership_24k_r3" \
  --target-visibility-plan "${TASK_VISIBILITY}"

TASK_STAGE="gate:occlusion_80_sentinels"
"${TASK_ENV}/bin/python" "${TASK_VERIFY}" \
  --replay-dir "${TASK_OUTPUT}/CL9V_occluder_ownership_24k_r3" \
  --historical-manifest "${TASK_HISTORICAL}" \
...
score_run marion_roll "${TASK_OUTPUT}/CL9V_marion_samefile_roll_24k_r3"
score_run marion_similarity "${TASK_OUTPUT}/CL9V_marion_fivepoint_similarity_24k_r3"
score_run occlusion_ownership "${TASK_OUTPUT}/CL9V_occluder_ownership_24k_r3"
