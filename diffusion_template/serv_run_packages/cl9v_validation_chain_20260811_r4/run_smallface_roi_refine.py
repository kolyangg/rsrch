...
def expanded_square_crop(bbox, width: int, height: int, ratio: float):
    x0, y0, x1, y1 = [float(value) for value in bbox]
    cx = (x0 + x1) / 2.0
    cy = (y0 + y1) / 2.0
    side = int(math.ceil(max(x1 - x0, y1 - y0) * ratio))
    side = min(max(1, side), width, height)
    ix0 = min(max(0, int(round(cx - side / 2.0))), width - side)
    iy0 = min(max(0, int(round(cy - side / 2.0))), height - side)
    ix1 = ix0 + side
    iy1 = iy0 + side
    if ix1 <= ix0 or iy1 <= iy0:
        raise ValueError(f"Invalid expanded square crop for bbox {bbox}")
    if ix1 - ix0 != iy1 - iy0:
        raise AssertionError("Branched-attention ROI crop must be square")
...
def main() -> None:
    ...
    parser.add_argument(
        "--seed-override",
        type=int,
        help="Labelled diagnostic seed for the 16 refined target rows only.",
    )
    ...
    for ordinal, index in enumerate(SMALL_FACE_INDICES):
        ...
        # 10 Aug 2026 - AICODE-NOTE: CL9's trained BA processors reshape the
        # target sequence as HxH. Preserve source aspect by taking a square
        # canvas crop; never feed a rectangular latent sequence into BA.
        work_side = round_up_8(max(256.0, crop.width * args.roi_scale))
        work_width = work_side
        work_height = work_side
        ...
        configured_seed = int(sample.get("seed", 0))
        seed = (
            configured_seed
            if args.seed_override is None
            else int(args.seed_override)
        )
        ...
        details = {
            ...
            "diagnostic_seed_exception": (
                args.seed_override is not None and seed != configured_seed
            ),
        ...
        print(
            "ROI_REFINEMENT_PROGRESS "
            f"completed={ordinal + 1}/{len(SMALL_FACE_INDICES)} "
            f"dataset_index={index} work_size={work_width}x{work_height}",
            flush=True,
        )
    ...
    write_json(
        output_dir / "run_manifest.json",
        {
            ...
            "bbox_expansion": args.bbox_expansion,
            "crop_policy": "square canvas around expanded face bbox",
            "late_steps": args.late_steps,
            "seed_override": args.seed_override,
            "diagnostic_seed_exception": (
                args.seed_override is not None and int(args.seed_override) != 0
            ),
            "feather_fraction": args.feather_fraction,
