...
def resolve_generation_bboxes(
    ...
    *,
    ...
    active_map_override: Path | None = None,
    ...
    if active_map_override is not None:
        active_path = active_map_override.resolve()
        if not active_path.is_file():
            raise FileNotFoundError(
                f"Explicit generation-bbox map is missing: {active_path}"
            )
        active_payload = json.loads(active_path.read_text(encoding="utf-8"))
        active_kind = "explicit_active_map"
    elif automatic:
        active_path = manual_path.with_name(f"{manual_path.stem}_auto.json")
        active_payload = json.loads(active_path.read_text(encoding="utf-8"))
        active_kind = "cached_auto"
    else:
        active_path = manual_path
        active_payload = manual_payload
        active_kind = "configured_map"
    ...
    for index, (sample, prompt, person_id) in enumerate(
        ...
        source = "manual_force" if force_manual else active_kind
        if bbox is None:
            # Index-keyed historical maps remain supported as a fallback.
            dataset_index = int(sample.get("_dataset_index", index))
            index_record = active_payload.get(f"{dataset_index:02d}.png")
    ...
    return bboxes, {
        ...
        "active_map_override": active_map_override is not None,
...
def subject_v2_id_embeddings(samples: list[dict]) -> tuple[torch.Tensor, list[dict]]:
    """Build box-owned ArcFace vectors for an explicit validation intervention."""
    ...
    audits = []
    for sample in samples:
        reference = first_reference(sample).convert("RGB")
        boxes, embeddings = aligner([reference])
        faces = [
            {"bbox": bbox, "embedding": embedding}
            for bbox, embedding in zip(boxes[0] or [], embeddings[0] or [])
        ]
        selected, audit = select_subject_face(
            faces,
            declared_bbox=sample.get("face_bbox_ref"),
            policy=BBOX_OVERLAP_V2,
        )
        result.append(torch.as_tensor(selected["embedding"]).float())
        audits.append(
            {
                "dataset_index": int(sample.get("_dataset_index", -1)),
                "identity": str(sample.get("id")),
                **audit.to_dict(),
            }
        )
    return torch.stack(result, dim=0), audits
...
def parse_sample_indices(raw: str | None, dataset_size: int) -> list[int]:
    ...
    if not values or values != sorted(set(values)):
        raise ValueError("--sample-indices must be a sorted unique integer list")
    if any(value < 0 or value >= dataset_size for value in values):
        raise ValueError(
            f"--sample-indices must be inside [0, {dataset_size - 1}]"
        )
    return values


def build_target_visibility_masks(
    plan_path: Path | None,
    *,
    samples: list[dict],
    generation_bboxes: list[list[float] | None],
    height: int,
    width: int,
    output_dir: Path,
) -> tuple[list[np.ndarray] | None, dict]:
    """Materialize a frozen image-space visibility plan for target queries."""
    ...
    if int(payload.get("schema_version", 0)) != 1:
        raise ValueError(f"Unsupported target visibility plan schema: {path}")
    if payload.get("coordinate_space") != "generation_bbox_fraction":
        raise ValueError(
            "target visibility plan coordinate_space must be "
            "generation_bbox_fraction"
        )
    entries = payload.get("entries")
    if not isinstance(entries, dict) or not entries:
        raise ValueError(f"Target visibility plan has no entries: {path}")

    masks_dir = output_dir / "target_visibility_masks"
    masks_dir.mkdir(exist_ok=True)
    masks: list[np.ndarray] = []
    rows = []
    selected_indices = {int(sample["_dataset_index"]) for sample in samples}
    unknown = sorted(int(key) for key in entries if int(key) not in selected_indices)
    if unknown:
        raise ValueError(
            "Target visibility plan contains indices outside this selection: "
            f"{unknown}"
        )
    ...
    def point_in_bbox(point, bbox):
        ...
        fx, fy = [float(value) for value in point]
        if not (0.0 <= fx <= 1.0 and 0.0 <= fy <= 1.0):
            raise ValueError(f"Normalized point is outside [0, 1]: {point!r}")
        x0, y0, x1, y1 = bbox
        return (
            int(round(x0 + fx * (x1 - x0))),
            int(round(y0 + fy * (y1 - y0))),
        )

    for sample, bbox in zip(samples, generation_bboxes):
        dataset_index = int(sample["_dataset_index"])
        entry = entries.get(str(dataset_index))
        ...
        if entry is not None:
            if bbox is None:
                raise ValueError(
                    f"Visibility entry {dataset_index} requires face_bbox_gen"
                )
            bbox = [float(value) for value in bbox]
            visible_ellipse = entry.get("visible_ellipse")
            if visible_ellipse is not None:
                if not isinstance(visible_ellipse, (list, tuple)) or len(visible_ellipse) != 4:
                    raise ValueError(
                        f"Entry {dataset_index} visible_ellipse must be [x0,y0,x1,y1]"
                    )
                draw.rectangle(
                    [point_in_bbox((0.0, 0.0), bbox), point_in_bbox((1.0, 1.0), bbox)],
                    fill=0,
                )
                draw.ellipse(
                    [
                        point_in_bbox(visible_ellipse[:2], bbox),
                        point_in_bbox(visible_ellipse[2:], bbox),
                    ],
                    fill=255,
                )
                operation_count += 1
            for rect in entry.get("exclude_rects", []):
                if not isinstance(rect, (list, tuple)) or len(rect) != 4:
                    raise ValueError(
                        f"Entry {dataset_index} exclude_rects items must be xyxy"
                    )
                draw.rectangle(
                    [point_in_bbox(rect[:2], bbox), point_in_bbox(rect[2:], bbox)],
                    fill=0,
                )
                operation_count += 1
            for polygon in entry.get("exclude_polygons", []):
                if not isinstance(polygon, (list, tuple)) or len(polygon) < 3:
                    raise ValueError(
                        f"Entry {dataset_index} exclusion polygon needs >=3 points"
                    )
                draw.polygon(
                    [point_in_bbox(point, bbox) for point in polygon],
                    fill=0,
                )
                operation_count += 1
            if operation_count == 0:
                raise ValueError(
                    f"Target visibility entry {dataset_index} has no operations"
                )

        mask = np.asarray(mask_image, dtype=np.float32) / 255.0
        masks.append(mask)
        mask_path = masks_dir / f"{dataset_index:03d}.png"
        if entry is not None:
        ...
        rows.append(
            {
                ...
                "modified": entry is not None,
                "operation_count": operation_count,
                "visible_fraction_full_canvas": float(mask.mean()),
                "mask_path": str(mask_path.resolve()) if entry is not None else None,
    ...
    return masks, {
        "enabled": True,
        "kind": "frozen_target_query_visibility",
        "plan_path": str(path),
        "plan_sha256": sha256_file(path),
        "coordinate_space": payload["coordinate_space"],
        "reference_kv_unchanged": True,
        "reference_mask_unchanged": True,
        "excluded_pixels_route": "native_background_lane",
        "rows": rows,
    }
...
def apply_reference_condition(
    ...
    if args.reference_condition == "matched":
        embedding_policy = str(
            getattr(args, "reference_id_embedding_policy", "legacy_first")
        ).lower()
        if embedding_policy == "bbox_overlap_v2":
            id_embeds, selection_audit = subject_v2_id_embeddings(samples)
        elif embedding_policy == "legacy_first":
            id_embeds, selection_audit = None, []
        else:
            raise ValueError(
                "reference_id_embedding_policy must be legacy_first or bbox_overlap_v2"
            )
        return matched_refs, matched_bboxes, id_embeds, {
            "kind": "matched",
            "scope": "PhotoMaker and spatial BA reference",
            "reference_id_embedding_policy": embedding_policy,
            "subject_selection_audit": selection_audit,
        }
...
def run(args) -> None:
    ...
    selected_indices = parse_sample_indices(
        getattr(args, "sample_indices", None), dataset_size
    )
    if not selected_indices:
        selected_indices = list(range(min(dataset_size, args.limit)))
        if len(selected_indices) != args.limit:
            raise ValueError(
                f"Requested {args.limit} samples, dataset returned {len(selected_indices)}"
            )
    samples = []
    for dataset_index in selected_indices:
        sample = dict(dataset[dataset_index])
        sample["_dataset_index"] = int(dataset_index)
        samples.append(sample)

    seed_override = getattr(args, "seed_override", None)
    seed_override_indices_raw = getattr(args, "seed_override_indices", None)
    if (seed_override is None) != (seed_override_indices_raw is None):
        raise ValueError(
            "--seed-override and --seed-override-indices must be supplied together"
        )
    seed_protocol = {
        "enabled": False,
        "configured_seeds_preserved": True,
        "diagnostic_exception": False,
    }
    if seed_override is not None:
        override_indices = parse_sample_indices(
            str(seed_override_indices_raw), dataset_size
        )
        selected_set = set(selected_indices)
        outside_selection = sorted(set(override_indices) - selected_set)
        if outside_selection:
            raise ValueError(
                "Seed override indices are outside the selected validation rows: "
                f"{outside_selection}"
            )
        configured_by_index = {}
        override_set = set(override_indices)
        for sample in samples:
            dataset_index = int(sample["_dataset_index"])
            configured_by_index[str(dataset_index)] = int(sample.get("seed", 0))
            if dataset_index in override_set:
                sample["seed"] = int(seed_override)
        # 11 Aug 2026 - AICODE-NOTE: Nonzero seeds are a labelled diagnostic
        # exception. Only declared target rows change seed; the remaining
        # full-96 sequence stays on seed 0 so historical RGB sentinels remain
        # meaningful and a seed sweep cannot silently alter the validation panel.
        seed_protocol = {
            "enabled": True,
            "configured_seeds_preserved": False,
            "diagnostic_exception": int(seed_override) != 0,
            "override_seed": int(seed_override),
            "override_dataset_indices": override_indices,
            "configured_seed_by_selected_index": configured_by_index,
            "unchanged_dataset_indices": sorted(selected_set - override_set),
        }
    ...
    generation_bboxes, generation_bbox_protocol = resolve_generation_bboxes(
        ...
        active_map_override=getattr(args, "generation_bbox_map", None),
    )
    ...
    target_visibility_masks, target_visibility_protocol = (
        build_target_visibility_masks(
            getattr(args, "target_visibility_plan", None),
            samples=samples,
            generation_bboxes=generation_bboxes,
            height=int(validation_kwargs.get("height", 1024)),
            width=int(validation_kwargs.get("width", 1024)),
    ...
    for start in range(0, len(samples), batch_size):
        ...
        if target_visibility_masks is not None:
            batch_kwargs["ba_target_visibility_mask"] = np.stack(
                target_visibility_masks[start:end], axis=0
            )
        ...
        completed = end
        seconds_per_image = elapsed / max(completed, 1)
        current_remaining = seconds_per_image * (len(samples) - completed)
        print(
            "VALIDATION_ETA "
            f"completed={completed}/{len(samples)} "
            f"elapsed_seconds={elapsed:.1f} "
            f"checkpoint_remaining_seconds={current_remaining:.1f}",
            flush=True,
        )
        ...
        per_image.append(
            {
                ...
                "dataset_index": int(sample.get("_dataset_index", index)),
                "filename": filename,
                "output_key": output_filename(prompts[index], person_ids[index]),
                "prompt": prompts[index],
                ...
                "target_visibility": (
                    target_visibility_protocol["rows"][index]
                    if target_visibility_masks is not None
                    else {"modified": False}
                ),
    ...
    manifest = {
        ...
        "target_visibility_protocol": target_visibility_protocol,
...
def main() -> None:
    ...
    parser.add_argument(
        "--sample-indices",
        help="Sorted comma-separated fixed-panel indices; overrides --limit.",
    )
    parser.add_argument(
        "--seed-override",
        type=int,
        help="Diagnostic seed applied only to --seed-override-indices.",
    )
    parser.add_argument(
        "--seed-override-indices",
        help="Sorted fixed-panel indices allowed to use --seed-override.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=["legacy_first", "bbox_overlap_v2"],
        default="legacy_first",
        help="Versioned PhotoMaker ArcFace subject selector for matched references.",
    )
    parser.add_argument(
        "--generation-bbox-map",
        type=Path,
        help=(
            "Exact active generation-bbox JSON captured for this historical run. "
            "Use when the run's cached auto map is not adjacent to the saved config."
        ),
    )
    parser.add_argument(
        "--target-visibility-plan",
        type=Path,
        help=(
            "Frozen image-space target-query visibility plan. Excluded pixels "
            "use the existing native lane while reference K/V remain unchanged."
        ),
    )
