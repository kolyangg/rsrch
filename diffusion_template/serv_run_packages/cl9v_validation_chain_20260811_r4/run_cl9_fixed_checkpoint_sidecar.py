...
def install_reference_transform(evaluator, manifest_path: Path, variant: str) -> None:
    ...
    def transformed_apply(eval_args, samples):
        ...
        if indices != list(range(96)):
            raise ValueError(
                "Marion transform requires the complete manual_val 0-95 sequence"
            )
        if {str(samples[index].get("id")).lower() for index in range(84, 96)} != {"marion"}:
            raise ValueError("manual_val indices 84-95 must all be Marion")

        refs, bboxes, embeds, intervention = original_apply(eval_args, samples)
        if len(refs) != 96 or len(bboxes) != 96:
            raise RuntimeError("Matched reference conditioning did not return 96 rows")
        # 10 Aug 2026 - AICODE-NOTE: The evaluator conditions all 96 rows before
        # its batch loop. Replace only the final Marion rows in those returned
        # arrays; rows 0-83 stay byte-for-byte on the historical input route.
        for position in range(84, 96):
            refs[position] = [transformed.copy()]
            bboxes[position] = list(bbox)
        ...
        intervention.update(
            {
                ...
                "modified_dataset_indices": list(range(84, 96)),
                "unchanged_sentinel_indices": list(range(84)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seed-override-indices")
...
def main() -> None:
    ...
    if (args.seed_override is None) != (args.seed_override_indices is None):
        raise ValueError(
    ...
    run_args = Namespace(
        ...
        seed_override_indices=args.seed_override_indices,
        batch_size=None,
