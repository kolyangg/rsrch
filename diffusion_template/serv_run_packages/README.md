...
K/V branched self-attention, disabled branched cross-attention, rank-128 BA,
effective generic/default adapters, `pose_adapt_ratio=0`, and the fixed-96
DDIM50/CFG5/RealVis validation contract.
...
The six concrete YAMLs are adapted from the matching `test`-branch packages.
...
CL14 needs the exact Cosmic inputs/hash. CL18 and CL19 additionally require the
sealed subject-v2 embeddings. CL20 also requires sealed BigCelebs and builds
