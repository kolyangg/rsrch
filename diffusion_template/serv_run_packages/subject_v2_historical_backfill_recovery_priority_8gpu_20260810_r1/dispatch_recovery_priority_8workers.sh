...
BACKFILL_TASK_NAME="${TASK_NAME}" BACKFILL_PACKAGE_ROOT="${PACKAGE_ROOT}" \
  ...
  BACKFILL_WAVE=recovery_and_priority_8gpu BACKFILL_WORKER_COUNT=8 \
  bash "${WORKER_SCRIPT}" &
pids=("$!")
for worker_index in 0 1 2 3 4 5 6; do
  ssh -p 2222 -o BatchMode=yes -o ConnectTimeout=20 \
    ...
    "BACKFILL_TASK_NAME='${TASK_NAME}' BACKFILL_PACKAGE_ROOT='${PACKAGE_ROOT}' BACKFILL_STAGING_ROOT='${STAGING_ROOT}' BACKFILL_WAVE=recovery_and_priority_8gpu BACKFILL_WORKER_COUNT=8 bash '${WORKER_SCRIPT}'" &
  pids+=("$!")
done
