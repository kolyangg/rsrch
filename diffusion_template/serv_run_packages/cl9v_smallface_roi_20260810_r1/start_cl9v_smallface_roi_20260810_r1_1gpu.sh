...
TASK_PACKAGE="/mnt/virtual_ai0001053-01309_SR006-nfs1/nasilaev/rsrch_test/diffusion_template/serv_run_packages/cl9v_smallface_roi_20260810_r1"
TASK_ROOT="${TASK_OWNER_ROOT}/analysis_sidecars/cl9v_smallface_roi_20260810_r1"
...
sha256sum "${TASK_CHECKPOINT}" "${TASK_HISTORICAL}" "${TASK_LEGACY_EMBEDS}" \
...
TASK_STAGE="generate:full96_historical_replay"
...
"${TASK_ENV}/bin/python" "${TASK_WRAPPER}" \
  ...
  --output-dir "${TASK_OUTPUT}/full96_historical_replay"

TASK_STAGE="gate:full96_historical_replay"
"${TASK_ENV}/bin/python" "${TASK_VERIFY}" \
  --replay-dir "${TASK_OUTPUT}/full96_historical_replay" \
  --historical-manifest "${TASK_HISTORICAL}" \
  --expect-count 96 \
  --output "${TASK_OUTPUT}/full96_historical_replay_verification.json"
...
run_roi() {
  ...
  "${TASK_ENV}/bin/python" "${TASK_ROI}" \
    ...
    --config "${TASK_CONFIG}" \
    --checkpoint "${TASK_CHECKPOINT}" \
    --baseline-dir "${TASK_OUTPUT}/full96_historical_replay" \
...
run_roi CL9V_smallface_roi_refine_24k_r1 18
run_roi CL9V_smallface_roi_refine_gentle_24k_r1 10
...
score_run full96_historical_replay "${TASK_OUTPUT}/full96_historical_replay"
score_run roi_refine_18 "${TASK_OUTPUT}/CL9V_smallface_roi_refine_24k_r1"
score_run roi_refine_10 "${TASK_OUTPUT}/CL9V_smallface_roi_refine_gentle_24k_r1"
