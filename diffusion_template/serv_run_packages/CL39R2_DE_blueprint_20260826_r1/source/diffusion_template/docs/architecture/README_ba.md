...
- [`src/model/photomaker_branched/_backup/branched_new2_clean.py`](../../src/model/photomaker_branched/_backup/branched_new2_clean.py) – historical training-time patcher snapshot that installs branched attention on the UNet
  ```python
...
- [`src/model/photomaker_branched/_old_attn_pr_model/attn_processor.py`](../../src/model/photomaker_branched/_old_attn_pr_model/attn_processor.py) – historical branched self-/cross-attention processor described for attn1 configs (`use_attn_v2: false`) and controlled by `pose_adapt_ratio`, `id_alpha`, `use_id_embeds`
  ```python
