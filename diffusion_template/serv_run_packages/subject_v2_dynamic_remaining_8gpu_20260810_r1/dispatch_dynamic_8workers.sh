...
TASK_NAME="subject_v2_dynamic_remaining_8gpu_20260810_r1"
...
BACKFILL_TASK_NAME="${TASK_NAME}" BACKFILL_PACKAGE_ROOT="${PACKAGE_ROOT}" \
  ...
  BACKFILL_WAVE=dynamic_remaining_8gpu BACKFILL_WORKER_COUNT=8 \
  bash "${WORKER_SCRIPT}" &
pids=("$!")
for worker_index in 0 1 2 3 4 5 6; do
  ssh -p 2222 -o BatchMode=yes -o ConnectTimeout=20 \
    -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    "${job_prefix}-mpiworker-${worker_index}" \
    "BACKFILL_TASK_NAME='${TASK_NAME}' BACKFILL_PACKAGE_ROOT='${PACKAGE_ROOT}' BACKFILL_STAGING_ROOT='${STAGING_ROOT}' BACKFILL_WAVE=dynamic_remaining_8gpu BACKFILL_WORKER_COUNT=8 bash '${WORKER_SCRIPT}'" &
  pids+=("$!")
done
