...
RUN_ID="rhca_cosmic_full_posefirst_speed_async_cpu_workers12_50"
...
if [[ "${CUDA_LAUNCH_BLOCKING:-0}" != "0" ]]; then
  echo "Asynchronous CPU-provider probe received CUDA_LAUNCH_BLOCKING=${CUDA_LAUNCH_BLOCKING}" >&2
  exit 73
fi

# The base environment intentionally falls back to CPU InsightFace. Do not
# prepend the ORT 1.20.1 CUDA overlay in this control.
python - <<'PY'
import onnxruntime as ort

if ort.__version__ != "1.16.0":
    raise RuntimeError(f"Unexpected control ONNX Runtime version: {ort.__version__}")
print("ONNX Runtime CPU-provider control:", ort.__version__, ort.get_available_providers())
PY
