...
def install_reference_transform(evaluator, manifest_path: Path, variant: str) -> None:
    ...
    def transformed_apply(eval_args, samples):
        ...
        if indices == list(range(84, 96)) and identities == {"marion"}:
            conditioned = []
        ...
        if any(index >= 84 for index in indices) or "marion" in identities:
            raise ValueError(
                "Marion transform requires indices 84-95 as the unchanged "
                "final batch of the full96 sequence"
            )
        # 10 Aug 2026 - AICODE-NOTE: Replay indices 0-83 unchanged so the final
        # Marion batch inherits the exact historical RNG and pipeline sequence.
        return original_apply(eval_args, samples)
