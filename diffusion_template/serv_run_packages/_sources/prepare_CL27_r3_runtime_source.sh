...
BASE_ROOT="${OWNER}/runtime_sources_cl27_cl29_v3/CL27_cosmic_frequency_surface_energy_24k_full96_r2"
BASE="${BASE_ROOT}/diffusion_template"
RUN="CL27_cosmic_frequency_surface_energy_24k_full96_r3"
ROOT="${OWNER}/runtime_sources_cl27_cl29_v4/${RUN}"
OVERLAY="${OWNER}/cl27_r3_overlay.tar"
PYTHON="${OWNER}/conda_env/photomaker_NS/bin/python"
test "$(sha256sum "${OVERLAY}" | cut -d' ' -f1)" = \
  "bcad9ce0f105a6e65db039a6d191e8441f205536f9e27253015feb3b727c109b"
...
for relative, expected in record["files"].items():
    ...
    if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest() != expected:
        raise RuntimeError(f"Changed sealed CL27 r2 source: {relative}")
PY
if [[ -e "${ROOT}" ]]; then
...
"${PYTHON}" "${ROOT}/diffusion_template/tools/verify_serv_source_manifest.py" build \
  ...
  --source-revision "cl27-no-grad-surface-guard-20260814-v4"
