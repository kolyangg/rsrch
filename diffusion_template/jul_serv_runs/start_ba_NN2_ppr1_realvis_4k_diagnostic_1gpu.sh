...
RUN_NAME="${RUN_NAME:-ba_NN2_ppr1_realvis_4k_diagnostic}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/ppr_4k_diagnostic}"
...
if [[ "${RUN_FOREGROUND:-0}" != "1" && "${DETACHED_RUN:-0}" != "1" ]]; then
  mkdir -p "${LOG_DIR}"
  echo "Starting NN2-PPR 4k A-E diagnostic matrix on GPU ${CUDA_VISIBLE_DEVICES}"
