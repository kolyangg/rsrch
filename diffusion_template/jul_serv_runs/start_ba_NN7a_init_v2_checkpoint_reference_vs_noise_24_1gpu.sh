...
# Complete RealVis causal matrix on the shared deterministic 24/96 subset.
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd -- "${SCRIPT_DIR}/.." && pwd)"
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${1:-}}"
if [[ -z "${CHECKPOINT_PATH}" || ! -f "${CHECKPOINT_PATH}" ]]; then
  echo "Pass the NN7a_init-v2 checkpoint-epoch2.pth as argument 1." >&2
  exit 2
fi
...
[[ "${CHECKPOINT_EPOCH}" == "2" ]] || {
  echo "The NN7a_init-v2 approval diagnostic expects epoch 2 (4k)." >&2
  exit 2
}
...
export MASTER_PORT="${MASTER_PORT:-29679}"
export NUM_EPOCHS=2
export RUN_NAME="${RUN_NAME:-ba_NN7a_init_v2_4000step_diagnostic24}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_DIR}/ppr_NN7a_init_v2_4000step_realvis_scale1_reference_vs_noise_subset${SUBSET_SIZE}_seed${SUBSET_SEED}}"

exec bash "${SCRIPT_DIR}/start_ba_NN7a_init_v2_1gpu.sh" \
