...
# Train for 4k with 96-image validations, then run the deterministic 24 matrix.
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd -- "${SCRIPT_DIR}/.." && pwd)"
TRAIN_RUN_NAME="${TRAIN_RUN_NAME:-ba_NN7a_init_v2_1gpu}"
...
[[ -f "${CHECKPOINT}" ]] || {
  echo "Missing NN7a_init-v2 4k checkpoint: ${CHECKPOINT}" >&2
  exit 3
}
...
bash "${SCRIPT_DIR}/start_ba_NN7a_init_v2_checkpoint_reference_vs_noise_24_1gpu.sh" \
  "${CHECKPOINT}" \
  "$@"
