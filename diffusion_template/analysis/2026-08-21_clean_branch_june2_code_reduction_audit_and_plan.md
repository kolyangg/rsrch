...
> **22 August implementation update:** the user revised the cleanup rule after
> this analysis: shared key files must retain the literal 2 June source,
> including apparent historical bloat. Sections 5-8 below are therefore the
> superseded pre-implementation proposal; the dependency inventory in Sections
> 2-4 is an audit snapshot and some module names moved during implementation.
> The implemented layout, exact
> insertion points, and a separate list of possible later bloat are recorded in
> [`2026-08-22_june2_key_file_rebase_implementation.md`](2026-08-22_june2_key_file_rebase_implementation.md).
...
| Model | `src.model.photomaker_branched.e13_model.PhotomakerBranchedLora` |
...
|---|---|---|
| Model entry and base | `src/model/photomaker_branched/e13_model.py`, `src/model/sdxl/original.py`, `src/model/photomaker_path.py` | Selected SDXL/PhotoMaker construction and training forward; historical `lora2.py` is retained unchanged |
| PhotoMaker identity encoder | `model_v2_NS.py`, `resampler.py`, `insightface_package.py` | PMv2 token/face conditioning |
| Shared hard BA | `attn_processor_cleanest.py`, `e13_attn_processor.py`, `branched_runtime.py`, `e13_runtime.py`, `branch_helpers.py`, `e13_training_helpers.py` | unchanged June equations plus selected target/reference doubled batch, target Q, reference K/V, masks, and processor installation |
...
| Pipelines | `photomaker_branched_clean.py`, `br_pipeline_helpers.py`, `photomaker_branched_subject_v2.py` | fixed CL14 generation and subject-v2 identity selection |
