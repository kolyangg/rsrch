...
: "${N3A_NEW_NUM_PROCESSES:?N3A_NEW_NUM_PROCESSES is required}"
: "${N3A_NEW_RUN_NAME:?N3A_NEW_RUN_NAME is required}"
: "${N3A_NEW_CUDA_DEVICES:?N3A_NEW_CUDA_DEVICES is required}"
: "${N3A_NEW_MASTER_PORT:?N3A_NEW_MASTER_PORT is required}"
...
(( GLOBAL_EFFECTIVE_BATCH % N3A_NEW_NUM_PROCESSES == 0 )) || \
    fail "GLOBAL_EFFECTIVE_BATCH must be divisible by N3A_NEW_NUM_PROCESSES"
LOCAL_EFFECTIVE_BATCH=$((GLOBAL_EFFECTIVE_BATCH / N3A_NEW_NUM_PROCESSES))
(( LOCAL_EFFECTIVE_BATCH % TRAIN_BATCH_SIZE == 0 )) || \
    fail "Local effective batch must be divisible by TRAIN_BATCH_SIZE"
...
cd "${PROJECT_DIR}"
log "N3a_new2: GPUs=${N3A_NEW_CUDA_DEVICES}; processes=${N3A_NEW_NUM_PROCESSES}"
log "physical/rank=${TRAIN_BATCH_SIZE}; accumulation=${ACCUM_STEPS}; global effective=${GLOBAL_EFFECTIVE_BATCH}"
log "Budget=$((STEPS_PER_EPOCH * NUM_EPOCHS)) optimizer steps; RealVis validation every ${STEPS_PER_EPOCH}"
...
CUDA_VISIBLE_DEVICES="${N3A_NEW_CUDA_DEVICES}" \
accelerate launch --config_file=src/configs/ddp/accelerate.yaml \
    --main_process_ip="${MASTER_ADDR}" \
    --main_process_port="${N3A_NEW_MASTER_PORT}" \
    --num_processes="${N3A_NEW_NUM_PROCESSES}" train.py \
    --config-name=one_id_ba_N3a_new2 \
    ...
    metrics=all_metrics \
    writer=cometml \
    writer.run_name="${N3A_NEW_RUN_NAME}" \
    "$@"
