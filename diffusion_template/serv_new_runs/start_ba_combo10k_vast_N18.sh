...
# ============================================================================================
# N18 = the WINNING recipe (combo: SA-only + ID loss) run to 10000 steps.
#   SA-only  : train_branched_ca_lora=false  (freeze the branched cross-attn = drift/melt pathway;
#              the essential stabilizer — ID loss with CA trained (N16) is unstable)
#   ID loss  : +model.use_id_loss=true w0.1   (identity-supervised objective; keeps improving)
#   10000 steps (trainer.n_epochs=10 x epoch_len=1000), val + checkpoint every 1000, bs=1.
# Why: on the 96-image full validation the combo (N14) is the best recipe (0.332) and beats the
# baseline on ALL 8 identities, and was still rising at 6k -> extend to 10k. Between the done 6k
# (N14) and the long 26k (N17). Rationale: debug_04Jul/08Jul_96val_all_runs.md.
# ~10-14 h at bs=1. Keep every 1k checkpoint; pick the best by the 96-image validation, not the last.
# ============================================================================================
...
#   writer.run_name -> ba_combo10k_N18
...
if ACCELERATE_LOG_LEVEL=error \
    ...
    accelerate launch --config_file=src/configs/ddp/accelerate.yaml --num_processes=1 train.py \
        ...
        metrics=all_metrics \
        writer=cometml writer.run_name="ba_combo10k_N18" \
        "$@"; then
