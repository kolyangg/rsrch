...
# ============================================================================================
# N14 = COMBINATION of the two winning levers (SA-only + ID loss), 6000 steps. PRIMARY next run.
#   SA-only  : train_branched_ca_lora=false  (freeze the branched cross-attn; N11 winner)
#   ID loss  : +model.use_id_loss=true w0.1   (identity-supervised objective; N13 winner)
#   6000 steps (trainer.n_epochs=6 x epoch_len=1000), bs=1 (ID-loss decode memory).
# Rationale/results: debug_04Jul/8Jul_results_N10_N11_N13_and_next.md §5. (The body below is the
# cloned N13 header — accurate for the ID-loss part; the SA-only + 6k are the additions above.)
# ============================================================================================
...
if ACCELERATE_LOG_LEVEL=error \
    ...
    accelerate launch --config_file=src/configs/ddp/accelerate.yaml --num_processes=1 train.py \
        ...
        metrics=all_metrics \
        writer=cometml writer.run_name="ba_combo_N14" \
        "$@"; then
