...
: "${NN6_NUM_PROCESSES:?NN6_NUM_PROCESSES is required}"
: "${NN6_RUN_NAME:?NN6_RUN_NAME is required}"
: "${NN6_CUDA_DEVICES:?NN6_CUDA_DEVICES is required}"
: "${NN6_MASTER_PORT:?NN6_MASTER_PORT is required}"
...
python -c 'from facenet_pytorch import InceptionResnetV1; InceptionResnetV1(pretrained="vggface2")' \
    >/dev/null || fail "NN6a requires facenet-pytorch and VGGFace2 weights"
...
NN6_VALIDATION_MODEL="SG161222/RealVisXL_V4.0"

(( GLOBAL_EFFECTIVE_BATCH % NN6_NUM_PROCESSES == 0 )) || \
    fail "GLOBAL_EFFECTIVE_BATCH must be divisible by NN6_NUM_PROCESSES"
LOCAL_EFFECTIVE_BATCH=$((GLOBAL_EFFECTIVE_BATCH / NN6_NUM_PROCESSES))
...
cd "${PROJECT_DIR}"
log "NN6a server: ${NN6_RUN_NAME}; GPUs=${NN6_CUDA_DEVICES}; processes=${NN6_NUM_PROCESSES}"
log "physical/rank=${TRAIN_BATCH_SIZE}; accumulation=${ACCUM_STEPS}; global effective=${GLOBAL_EFFECTIVE_BATCH}"
log "Approval budget=$((STEPS_PER_EPOCH * NUM_EPOCHS)) optimizer steps; RealVis validation every ${STEPS_PER_EPOCH}"
...
CUDA_VISIBLE_DEVICES="${NN6_CUDA_DEVICES}" \
accelerate launch --config_file=src/configs/ddp/accelerate.yaml \
    --main_process_ip="${MASTER_ADDR}" \
    --main_process_port="${NN6_MASTER_PORT}" \
    --num_processes="${NN6_NUM_PROCESSES}" train.py \
    --config-name=one_id_ba_NN6a_factorized_identity_only_up0 \
    ...
    pretrained_model_for_validation_name_or_path="${NN6_VALIDATION_MODEL}" \
    metrics=all_metrics \
    writer=cometml \
    writer.run_name="${NN6_RUN_NAME}" \
    "$@"
