...
# N7: blended, LOW face weight. λ×noise 2x2 around N6 (blended anchor).
# Design: debug_04Jul/7Jul_experiments_analysis.md §5.
#   corner = (lambda_face=0.05, ba_noise_lr_scale=0.1)
# Hypothesis: lowering the face-crop weight (0.15 -> 0.05) reduces the prop-melt (Skiing/Kickboxing)
# and keeps easy frontal cases nearer the untrained step-0 quality (which trained runs degrade).
#
# vs N6 (start_ba_nr_blend_vast_N6.sh): lambda_face 0.15 -> 0.05 ; run_name -> ba_nr_blend_N7.
# Everything else identical: blended_masked, noise_and_ref, ba_noise_lr_scale=0.1, clean ref
...
if ACCELERATE_LOG_LEVEL=error \
    ...
    accelerate launch --config_file=src/configs/ddp/accelerate.yaml --num_processes=1 train.py \
        ...
        metrics=all_metrics \
        writer=cometml writer.run_name="ba_nr_blend_N7" \
        "$@"; then
