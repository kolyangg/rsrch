...
: "${NN3B_NUM_PROCESSES:?NN3B_NUM_PROCESSES is required}"
: "${NN3B_RUN_NAME:?NN3B_RUN_NAME is required}"
: "${NN3B_CUDA_DEVICES:?NN3B_CUDA_DEVICES is required}"
: "${NN3B_MASTER_PORT:?NN3B_MASTER_PORT is required}"
...
python -c 'from facenet_pytorch import InceptionResnetV1; InceptionResnetV1(pretrained="vggface2")' \
    >/dev/null || fail "NN3b requires facenet-pytorch and cached/downloadable VGGFace2 weights"
...
cd "${PROJECT_DIR}"
log "NN3b server run: name=${NN3B_RUN_NAME} GPUs=${NN3B_CUDA_DEVICES} processes=${NN3B_NUM_PROCESSES}"
log "Dataset=cosmic_large (NFS paths), batch/rank=${TRAIN_BATCH_SIZE}, global_batch=$((TRAIN_BATCH_SIZE * NN3B_NUM_PROCESSES))"
...
CUDA_VISIBLE_DEVICES="${NN3B_CUDA_DEVICES}" \
accelerate launch --config_file=src/configs/ddp/accelerate.yaml \
    --main_process_ip="${MASTER_ADDR}" \
    --main_process_port="${NN3B_MASTER_PORT}" \
    --num_processes="${NN3B_NUM_PROCESSES}" train.py \
    --config-name=one_id_ba_NN3b_learned_null_pm_attenuation \
