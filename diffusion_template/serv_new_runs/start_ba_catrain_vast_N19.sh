...
# ============================================================================================
# N19 = CROSS-ATTENTION-TRAINABLE long run (15000 steps). Tests the user's hypothesis that a
# trainable branched cross-attn improves face<->pose fit (N17/combo freezes CA -> stiffer face).
#
#   train_branched_ca_lora=TRUE   -> the branched cross-attn (attn2) trains. Crucially, with
#     ba_noise_lr_scale=0.05 the CA-*ref* pathway (ref_to_k/v = face conditioning, the part that
#     helps pose/blend) trains at full LR, while the CA-*noise* pathway (noise_to_v = the whole-image
#     drift/melt channel, §4.2) is damped to 0.05x -> selective: keep the useful CA, starve the drift.
#   +model.id_loss_weight=0.05    -> halved vs the combo (0.1). N16 showed ID-loss + CA-trained is
#     unstable; lowering the ID pressure + hard noise damping (0.05) + grad-clip 1.0 is the
#     stabilization. (If it still degrades like N16, CA-trained is fundamentally unstable here.)
#   Everything else = the combo recipe: blended λ0.15, noise_and_ref, lr 1e-4, clean ref, wd 1e-3,
#   clip 1.0, warmup 200, uncond_face_fix, id_only, RealVis val, bs=1.
#   15000 steps (n_epochs=15 x epoch_len=1000), val + checkpoint EVERY 1000 (fine, to catch any
#   N16-style degradation early and pick the best checkpoint). ~15-20 h at bs=1.
#
# NB the more DIRECT lever for "pose doesn't fit the face" is POSE_ADAPT_RATIO (hardcoded 0.0 in
# attn_processor_cleanest.py -> pure frontal ref face, no pose blend). Raising it (~0.2-0.3) lets the
# face adapt to the body pose but trades identity, and needs a small code change to un-hardcode it.
# CA-training (this run) helps face conditioning/blend; it may not fully fix pose ORIENTATION.
# ============================================================================================
...
#   +model.id_loss_weight=0.05            weight on the ID term. Smoke showed weight 0.5 makes the ID
...
#   writer.run_name -> ba_catrain_N19
...
if ACCELERATE_LOG_LEVEL=error \
    ...
    accelerate launch --config_file=src/configs/ddp/accelerate.yaml --num_processes=1 train.py \
        ...
        metrics=all_metrics \
        writer=cometml writer.run_name="ba_catrain_N19" \
        "$@"; then
