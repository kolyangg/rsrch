...
# N8: blended, MORE noise integration. λ×noise 2x2 around N6 (blended anchor).
# Design: debug_04Jul/7Jul_experiments_analysis.md §5.
#   corner = (lambda_face=0.15, ba_noise_lr_scale=0.25)
# Hypothesis: raising the noise-pathway LR (0.1 -> 0.25) trains the body/gen branch more, improving
# face<->body integration on hard poses (N5=0.0 < N6=0.1; extend the noise axis up). Keeps N6's λ.
#
# vs N6 (start_ba_nr_blend_vast_N6.sh): ba_noise_lr_scale 0.1 -> 0.25 ; run_name -> ba_nr_blend_N8.
# Everything else identical: blended_masked, noise_and_ref, lambda_face=0.15, clean ref
...
if ACCELERATE_LOG_LEVEL=error \
    ...
    accelerate launch --config_file=src/configs/ddp/accelerate.yaml --num_processes=1 train.py \
        ...
        metrics=all_metrics \
        writer=cometml writer.run_name="ba_nr_blend_N8" \
        "$@"; then
