...
N29_CHECKPOINT="${N29_CHECKPOINT:-${PROJECT_DIR}/saved/ba_qformer_idtokens_N29/checkpoint-epoch5.pth}"
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/logs_new_runs}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/${RUN_NAME}_$(date +%Y%m%d_%H%M%S).log}"

if [[ ! -f "${N29_CHECKPOINT}" ]]; then
    echo "N29 checkpoint not found: ${N29_CHECKPOINT}" >&2
...
if [[ "${RUN_FOREGROUND:-0}" != "1" && "${DETACHED_RUN:-0}" != "1" ]]; then
    ...
    echo "Resuming: ${N29_CHECKPOINT}"
    echo "Log: ${LOG_FILE}"
    DETACHED_RUN=1 LOG_DIR="${LOG_DIR}" LOG_FILE="${LOG_FILE}" \
        N29_CHECKPOINT="${N29_CHECKPOINT}" \
...
accelerate launch --config_file=src/configs/ddp/accelerate.yaml \
    ...
    --config-name=one_id_ba_qformer_continue20k_N33 \
    ...
    trainer.n_epochs=20 \
    continue_run=true \
    saved_checkpoint="${N29_CHECKPOINT}" \
