...
# N34: corrected high-resolution QFormer residual with decoded causal identity
# supervision. Four ranks, local batch 1, accumulation 2 -> global batch 8.

RUN_NAME="${RUN_NAME:-ba_causal_highres_qformer_4gpu_N34}"
...
if [[ "${RUN_FOREGROUND:-0}" != "1" && "${DETACHED_RUN:-0}" != "1" ]]; then
    mkdir -p "${LOG_DIR}"
    echo "Starting ${RUN_NAME} detached on GPUs ${CUDA_VISIBLE_DEVICES:-0,1,2,3}"
    echo "Log: ${LOG_FILE}"
...
echo "N34: 6 up_blocks.1 CA sites, FP32 BA masters, post-CFG delta, QFormer memory"
