...
# N38: N36 identity-owner route with bbox-normalized QFormer memory.
# Two ranks, local batch 1, accumulation 2 -> global effective batch 4.

RUN_NAME="${RUN_NAME:-ba_identity_owner_cropped_qformer_2gpu_N38}"
...
echo "N38: N36 route with bbox-normalized QFormer identity memory"
...
accelerate launch --config_file=src/configs/ddp/accelerate.yaml \
    ...
    --config-name=one_id_ba_identity_owner_cropped_qformer_N38 \
