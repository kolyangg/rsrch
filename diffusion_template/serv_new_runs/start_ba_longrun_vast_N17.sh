...
# ============================================================================================
# N17 = LONG RUN of the best recipe (combo: SA-only + ID loss), 26000 steps.
#   SA-only  : train_branched_ca_lora=false  (freeze the branched cross-attn = the drift/melt
#              pathway; it is the STABILIZER — ID loss alone with CA trained (N16) is unstable)
#   ID loss  : +model.use_id_loss=true w0.1   (identity-supervised objective; keeps improving)
#   26000 steps (trainer.n_epochs=13 x epoch_len=2000), val + checkpoint every 2000, bs=1.
# Why this config: at 6k, N14 (combo) reached the top id-sim AND was still rising, while N15
# (SA-only alone) had plateaued and N16 (ID loss alone) was unstable -> the combo has the most
# headroom for a long run. Rationale/results: debug_04Jul/7Jul_N14_N15_N16_longrun.md.
# Expect ~24-30 h at bs=1. Pick the best checkpoint by the id-sim curve (and the 96-image full
# validation) rather than assuming the last one is best.
# Faster (bs=2) alternative if time-constrained: SA-only alone (start_ba_saonly6k_vast_N15.sh with
# n_epochs raised) — nearly as good at 6k but was plateauing.
# ============================================================================================
...
#   writer.run_name -> ba_longrun_N17
...
if ACCELERATE_LOG_LEVEL=error \
    ...
    accelerate launch --config_file=src/configs/ddp/accelerate.yaml --num_processes=1 train.py \
        ...
        metrics=all_metrics \
        writer=cometml writer.run_name="ba_longrun_N17" \
        "$@"; then
