#!/bin/bash
# setup_pulid_branched.sh - Setup script to integrate branched attention into PuLID

set -e

echo "Setting up PuLID with Branched Attention support..."

# Check if we're in the right directory structure
if [ ! -d "PhotoMaker" ] || [ ! -d "PuLID" ]; then
    echo "Error: This script should be run from the parent directory containing both PhotoMaker and PuLID folders"
    exit 1
fi

# Copy necessary utilities from PhotoMaker to PuLID
echo "Copying mask utilities from PhotoMaker to PuLID..."
...
for file in "${FILES_TO_COPY[@]}"; do
    if [ -f "PhotoMaker/photomaker/$file" ]; then
        cp "PhotoMaker/photomaker/$file" "PuLID/pulid/"
        echo "  Copied $file"
    else
        echo "  Warning: $file not found in PhotoMaker/photomaker/"
    fi
done

# Create a symbolic link for backward compatibility if needed
if [ ! -L "PuLID/pulid/photomaker" ]; then
    ln -s "../../PhotoMaker/photomaker" "PuLID/pulid/photomaker"
    echo "Created symbolic link to PhotoMaker modules"
fi

# Create modified id_grid.sh for PuLID
cat > "PuLID/id_grid_pulid.sh" << 'EOF'
#!/usr/bin/env bash
...
chmod +x "PuLID/id_grid_pulid.sh"
echo "Created PuLID/id_grid_pulid.sh"

# Create a test configuration if it doesn't exist
if [ ! -f "PuLID/id_grid_full.json" ]; then
    cat > "PuLID/id_grid_full.json" << 'EOF'
{
  "rows": [
    {
  ...
  "columns": [
    {
      "slug": "nobr",
      "use_branched_attention": 0,
      "face_embed_strategy": "id_embeds"
    },
    {
      "slug": "id_embeds",
      "use_branched_attention": 1,
      "face_embed_strategy": "id_embeds",
...
EOF
    echo "Created sample configuration: PuLID/id_grid_full.json"
fi

echo ""
echo "Setup complete! You can now run PuLID with branched attention:"
echo ""
echo "  cd PuLID"
echo "  ./id_grid_pulid.sh id_grid_full.json"
echo ""
echo "Or run individual inference:"
echo ""
echo "  python pulid_generate_NS2.py \\"
echo "    --image_folder ../compare/testing/references \\"
echo "    --prompt_file ../compare/testing/prompt_one2.txt \\"
echo "    --class_file ../compare/testing/classes_ref.json \\"
echo "    --output_dir outputs \\"
echo "    --use_branched_attention \\"
echo "    --auto_mask_ref"
echo ""