...
OUTPUT_ROOT=${5:-../compare/results/PM_upgrade4_Nov}
...
# Parse scenarios from JSON into "pms,ms,bas,mode" lines
mapfile -t SCENARIOS < <(python3 - "$JSON_FILE" <<'PY'
import json,sys
j=json.load(open(sys.argv[1]))
for r in j.get("runs", []):
    bsm = r.get("branched_start_mode","both").lower()
    pms = int(r.get("photomaker_start_step", 10))
    ms  = int(r.get("merge_start_step", 10))
    bas = int(r["branched_attn_start_step"])
    print(f"{pms},{ms},{bas},{bsm}")
PY
)

# Run each scenario into its own subfolder for clean aggregation
for pair in "${SCENARIOS[@]}"; do
  IFS=, read -r PMS MS BAS MODE <<< "$pair"
  SUBDIR="$OUTPUT_ROOT/pms${PMS}_ms${MS}_bas${BAS}_${MODE}"
  mkdir -p "$SUBDIR"
  echo "[RUN] photomaker_start_step=$PMS  merge_start_step=$MS  branched_attn_start_step=$BAS  branched_start_mode=$MODE  → $SUBDIR"
...
for r in scenarios:
    ...
    bas = int(r["branched_attn_start_step"])
    bsm = r.get("branched_start_mode","both").lower()
    label = f"\"photomaker_start_step\": {pms}, \"merge_start_step\": {ms}, \"branched_attn_start_step\": {bas}, \"branched_start_mode\": \"{bsm}\""
    subdir = Path(output_root) / f"pms{pms}_ms{ms}_bas{bas}_{bsm}"
...
pdf_path = out_dir / "pm_vs_ba_steps_02Sep.pdf"   # matches the existing message format
