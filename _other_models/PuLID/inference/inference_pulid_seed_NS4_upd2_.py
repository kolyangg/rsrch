...
def main():
    ...
    for ref_path in refs:
        ...
        #         _ns_tmp = PuLIDPipelineNS()
        #         uncond_id_embedding, id_embedding = _ns_tmp.get_id_embedding([np_ref])
        #         use_v11_api = True
        #         del _ns_tmp
        #     except Exception as _e_emb:
        #         print(f"[BrNS] Warning: NS id embedding failed ({_e_emb}); falling back to base encoder")
        # if not use_v11_api:
        #     try:
        #         # base v1.0 API
        ...
        for p_idx, prompt in enumerate(prompts):
            ...
            for img_id in [TARGET_IMG_ID]:
                ...
                #     # v1.1 style
                #     try:
                #         # Prefer NS pipeline when branched attention is requested and available
                #         if bool(args.use_branched_attention):
                #             # Force NS branched pipeline path; if it fails, raise to stop execution
                #             from pulid.pipeline_NS import PuLIDPipeline as PuLIDPipelineNS  # type: ignore
                #             ns = PuLIDPipelineNS()
                ...
                #             # proceed to debug saving below
                #         else:
                #             # Non-branched path (single image with TARGET_IMG_ID seed semantics)
                #             def _reseed(s: int):
                #                 os.environ["PL_GLOBAL_SEED"] = str(s)
                #                 torch.manual_seed(s)
                #                 if torch.cuda.is_available():
                #                     torch.cuda.manual_seed_all(s)
                #                 random.seed(s)
                #                 np.random.seed(s)
                #             # _reseed(int(args.seed) + TARGET_IMG_ID)
                ...
                #             # Non-branched path: reseed per image for parity with old CLI
                #             # def _reseed(s: int):
                #             #     os.environ["PL_GLOBAL_SEED"] = str(s)
                #             #     torch.manual_seed(s)
                #             #     if torch.cuda.is_available():
                ...
                if use_v11_api:
                    ...
                    try:
                        ...
                        img = imgs[0]
                        # else:
                        #     # Two-pass generation with temporary self-attn patch; blend by face mask
                        #     def _reseed(s:int):
                        #         os.environ["PL_GLOBAL_SEED"]=str(s); torch.manual_seed(s)
                        #         if torch.cuda.is_available(): torch.cuda.manual_seed_all(s)
                        #         random.seed(s); np.random.seed(s)
                        #     _reseed(int(args.seed)+int(img_id))
                        # (removed duplicated non-branched block; single path already executed above)
                            # img_emb = image_embedding if image_embedding is not None else base.get_id_embedding(np_ref)
                            ...
                            #         # Avoid warnings from diffusers about unexpected kwargs
                            ...
                            # # Blend
                            # # Use imported mask for blending instead of oval fallback
                            # try:
                            #     mask_pil = Image.open(import_mask).convert('L').resize((args.width,args.height))
                            # except Exception:
                            ...
                            # base_np=np.array(img_base); face_np=np.array(img_face); mask_np=(np.array(mask_pil)>127)[:,:,None].astype(np.float32)
                            # blend_np=(base_np*(1.0-mask_np)+face_np*mask_np).astype(np.uint8)
                            # img=Image.fromarray(blend_np)
                    except TypeError:
                        # Older signature without branched flag
                        # img_emb = image_embedding if image_embedding is not None else base.get_id_embedding(np_ref)
                        # imgs = base.inference(
                        #     prompt=cur_prompt,
                        #     size=(1, args.height, args.width),
