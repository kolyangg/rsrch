...
class PhotoMakerStableDiffusionXLPipeline2(StableDiffusionXLPipeline):
    ...
    def __call__(
        ...
        mask_interval: int = 5,
        # ---------------- mask I/O ----------------
        export_mask: bool = False, # True, # False,              # save final face-mask to disk
        ...
        **kwargs,
        ...
        # ─────────────────────────────────────────────────────────────
        #  Encode the FIRST id-image once → latent grid (B=1,C=4,H/8,W/8)
        # ─────────────────────────────────────────────────────────────
        # if use_branched_attention:
        #     ref_img_for_lat = input_id_images[0]           # first reference
        #     if isinstance(ref_img_for_lat, PIL.Image.Image):
        #         ref_img_for_lat = (
        #             np.array(ref_img_for_lat).astype(np.float32) / 255.0
        #         )
        #         ref_img_for_lat = torch.from_numpy(ref_img_for_lat).permute(2,0,1)

        #     # ------------------------------------------------------------
        #     #  Convert to the same dtype as the VAE weights (bf16/fp16/fp32)
        #     #  and to the range expected by SD-VAE  ->  [-1, 1]
        #     # ------------------------------------------------------------
        #     vae_dtype = next(self.vae.parameters()).dtype     # e.g. torch.bfloat16

        #     if ref_img_for_lat.dtype == torch.uint8:
        #         ref_img_for_lat = ref_img_for_lat.float() / 255.0

        #     ref_img_for_lat = ref_img_for_lat * 2.0 - 1.0      # [0,1] → [-1,1]
        #     ref_img_for_lat = ref_img_for_lat.to(device=device, dtype=vae_dtype)
        #     ref_img_for_lat = ref_img_for_lat.unsqueeze(0)     # (1,3,H,W)


        # if use_branched_attention:
        #     # reuse the same pixel tensor already fed to id-encoder
        #     ref_img_for_lat = id_pixel_values[0].to(device)     # (3,H,W) in [-1,1]
        #     if ref_img_for_lat.dim() == 3:
        #         ref_img_for_lat = ref_img_for_lat.unsqueeze(0)  # → (1,3,H,W)

        #     ref_img_for_lat = ref_img_for_lat.to(
        #         dtype=next(self.vae.parameters()).dtype         # match VAE weights
        #     )

        #     # with torch.no_grad():
        #     #     mu = self.vae.encode(ref_img_for_lat).latent_dist.mode()
        #     #     _ref_latents_all = mu * self.vae.config.scaling_factor  # (1,4,h,w)
            
        #     try:
        #         with torch.no_grad():
        #             mu = self.vae.encode(ref_img_for_lat).latent_dist.mode()
        #             _ref_latents_all = (
        #                 mu * self.vae.config.scaling_factor
        #             )  # (1,4,h,w)
        #             print(f"[DBG] VAE ref-latents ready  shape={_ref_latents_all.shape}")
        #     except Exception as e:
        #         print(f"[WARN] VAE encode failed: {e}  → will fall back to noisy latents")
        ...
        # # ------------------------------------------------------------------ #
        # # 6.a  ⟶  Reference-latents for branched-attention  (now safe)
        # # ------------------------------------------------------------------ #
        # if use_branched_attention:
        #     ref_img_for_lat = id_pixel_values[0:1].to(               # (1,3,H,W)  in [-1,1]
        ...
        if use_branched_attention:
            ...
            _ref_latents     = None                   # face-region K/V
        ...
        if use_branched_attention:
            ...
            def _patch_self_attn(pipeline_self):
                ...
                def custom_attn_forward(mod, hidden_states,
                    ...
                    # # project queries from current latent
                    # q = (mod.to_q if hasattr(mod, "to_q") else mod.q_proj)(
                    #     hidden_states)


                    # ref = pipeline_self._kv_override
                    # # Accept either spatial latents (B,C,H,W) **or** token tensor (B,L,C)
                    # if ref is not None and ref.dim() == 4:          # (B,C,H,W)
                    #     ref = ref.permute(0, 2, 3, 1)               # → (B,H,W,C)
                    #     ref = ref.reshape(ref.shape[0], -1, ref.shape[3])  # (B,L,C)

                    # # -- ensure channel-dimension matches this block --
                    # C_tgt = hidden_states.shape[-1]
                    # C_src = ref.shape[-1]
                    # if C_src < C_tgt:                      # pad zeros up to C_tgt
                    #     pad = (0, C_tgt - C_src)          # (left,right) on last dim
                    #     ref = F.pad(ref, pad)
                    # elif C_src > C_tgt:                   # crop if accidentally larger
                    #     ref = ref[..., :C_tgt]


                    # k = (mod.to_k if hasattr(mod, "to_k") else mod.k_proj)(ref)
                    # v = (mod.to_v if hasattr(mod, "to_v") else mod.v_proj)(ref)

                    # # ------------------------------------------------------
                    # #  build reference K/V once per call
                    # # ------------------------------------------------------
                    # C_tgt = hidden_states.shape[-1]

                    # if pipeline_self._id_embed_vec is not None:
                    #     # Arc-Face / CLIP vector → pad or crop to d
                    ...
                    # else:
                    #     # fall back to latent snapshot if identity vec missing
                    #     ref = pipeline_self._kv_override
                    #     if ref.dim() == 4:                               # (B,C,H,W)
                    #         ref = ref.permute(0,2,3,1).reshape(1,-1,ref.shape[1])  # (1,L,C)
                    #     if ref.shape[-1] != C_tgt:
                    #         ref = F.pad(ref, (0, C_tgt - ref.shape[-1]))
                    ...
                    k = k.view(1, k.shape[1], h, d).permute(0, 2, 1, 3) # 1 h Lr d
                    v = v.view(1, v.shape[1], h, d).permute(0, 2, 1, 3) # 1 h Lr d
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if branched_now:
                    # initialise once (first step after mask is built)
                    # if not _branched_ready:
                    #     _ref_latents    = latents.detach().clone()
                    ...
                    if not _branched_ready:
                        ...
                        #     # fallback: use a clone of current latents
                        #     _ref_latents_all = latents.detach().clone()
                        #     print("[WARN] _ref_latents_all was None – "
                        #           "falling back to noisy latents")
                        ...
                        # mask-out background → keep only face tokens
                        # mask_bool = torch.from_numpy(self._face_mask).to(
                        #     device, dtype=torch.bool
                        # ).view(1,1,*latents.shape[-2:])
                        ...
                        # _ref_latents = _ref_latents_all.clone()
                        # _ref_latents[~mask_bool] = 0.0
                        ...
                        _ref_latents = _ref_latents_all.clone()
                        _ref_latents[~mask_4ch] = 0.0


                        print(f"[DBG] reference latents prepared  "
                              f"mean={_ref_latents.mean():.4f}")
                        _branched_ready = True
                    ...
                    # ── face branch – pass frozen KV snapshot ─────────────
                    # added_cond_face = dict(added_cond_kwargs)
                    # added_cond_face["kv_override"] = _ref_latents           # handled inside patched attention

                    # face branch – set override
                    self._kv_override = _ref_latents
                    ...
                    if i == branched_attn_start_step:     # one-time check
                        print(f"[DBG] mask-true ratio = {mask_bool.float().mean():.3f}  "
                              f"face-K/V non-zero = "
                              f"{(_ref_latents.abs()>0).float().mean():.3f}")
                            ...
                            if encoder_hidden_states is None and hasattr(self, "_face_mask") and self._face_mask is not None:
                                ...
                                if L_face > 0:
                                    ...
                                    if face_embed_strategy.lower() == "heatmap":
                                        ...
                                        in_feats = ref_vec.shape[-1]                 # eg. 512

                                        # ---- deterministic pad / crop instead of random weight ----
                                        if in_feats < d:
                                            repeat = (d + in_feats - 1) // in_feats  # ceil-div
                                            ref_pad = ref_vec.repeat(1, repeat)[..., :d]
                                        else:
                                            ref_pad = ref_vec[..., :d]

                                        K_ref = ref_pad.to(dtype=Q.dtype)   # (1, d)
                                        V_ref = K_ref  
        ...
        if export_mask and hasattr(self, "_face_mask") and self._face_mask is not None:
            Path(mask_save_dir).mkdir(parents=True, exist_ok=True)
            out_path = os.path.join(mask_save_dir, "mask_export.png")
