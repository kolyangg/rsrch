    ...
    def __call__(
        ...
        **kwargs,
        ...
        if use_branched_attention:
            _branched_ready = False     # becomes True once mask & ref-latents exist
            _ref_latents    = None      # frozen latents used as KV for the face branch
            ...
            def _patch_self_attn(pipeline_self):
                ...
                def custom_attn_forward(mod, hidden_states,
                    ...
                    q = (mod.to_q if hasattr(mod, "to_q") else mod.q_proj)(
                        hidden_states)
                    # project K,V from the *reference* latents snapshot
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # _hook_alive = any(name in orig_attn_forwards for name in wanted_layers)
                # print(f"[DBG] hook alive? {_hook_alive}  wanted={wanted_layers}")


                # show whether any attention hooks are still alive
                if "orig_attn_forwards" in locals():
                    _hook_alive = bool(orig_attn_forwards)
                    print(f"[DBG] hook alive? {_hook_alive}")
