        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if branched_now:
                    ...
                    # # --- add *matching noise* to reference latents ----------
                    # ref_kv_t = self.scheduler.add_noise(
                    #     _ref_latents_clean,
                    #     torch.randn_like(_ref_latents_clean),
                    #     t.expand(_ref_latents_clean.shape[0]),
                    # )

                    # ref_kv_t = ref_kv_t * mask_4ch        # ① keep background really zero
                    # ref_kv_t = ref_kv_t * 0.2             # ② down-scale K/V → same σ as Q (empirical)

                    # if not hasattr(self, "_dbg_scale_once"):
                    #     print("[DBG σ] t=", t.item(),
                    #           "Q=", latent_model_input.std().item(),
                    #           "K/V=", ref_kv_t.std().item())
                    #     self._dbg_scale_once = True


                    # ref_kv_t = ref_kv_t * mask_4ch        # keep background zero

                    # # --- auto-scale K/V to match Q ---
                    # q_std  = latent_model_input.std(dim=(1,2,3), keepdim=True)
                    # kv_std = ref_kv_t.std(dim=(1,2,3), keepdim=True).clamp(min=1e-4)
                    # ref_kv_t = ref_kv_t * (q_std / kv_std)


                    # ① add identical noise each step (matches scheduler σ_t)
                    ...
                    ref_kv_t = ref_kv_t * mask_4ch


                    if not hasattr(self, "_dbg_scale_once"):
                        all_kv = ref_kv_t.std().item()

                        # σ over *face tokens only* (expand mask for CFG=2)
                        _mask = mask_4ch.expand(ref_kv_t.shape[0], -1, -1, -1)
                        face_std = ref_kv_t[_mask].std().item()

                        print(f"[DBG σ]   t={t.item():>4}  "
                              f"Q={q_std.mean().item():.3f}  "
                              f"K/V(global)={all_kv:.3f}  "
                              f"K/V(face)={face_std:.3f}")
                        self._dbg_scale_once = True
                    ...
                    #           f"{(ref_kv_t.abs()>0).float().mean():.3f}")
