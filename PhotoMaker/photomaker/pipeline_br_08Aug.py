...
from .branched_new import (
    ...
    encode_face_prompt,
    create_face_mask_from_attention,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ##### NEW BRANCHED ATTENTION LOGIC #####
                if use_branched_attention and i >= branched_attn_start_step:
                    # Build noise & ref masks from imports/heatmaps every step (keeps shapes consistent)
                    aggregate_heatmaps_to_mask(self, mask_mode, import_mask,      suffix="")
                    mask4     = prepare_mask4(self, latent_model_input, suffix="")
                    aggregate_heatmaps_to_mask(self, mask_mode, import_mask_ref,  suffix="_ref")
                    mask4_ref = prepare_mask4(self, latent_model_input, suffix="_ref")
                    ...
                    noise_pred, noise_face, noise_bg = two_branch_predict(
                        ...
                        mask4_ref=mask4_ref,
                        t=t,
                        prompt_embeds_step=current_prompt_embeds,
        ...
        ## CLEANUP ##
        if use_branched_attention:
            # # Restore original processors only after ALL denoising steps
            # if hasattr(self, '_original_attn_processors'):
            #    print("[Cleanup] Restoring original processors after denoising")
            #    self.unet.set_attn_processor(self._original_attn_processors)

            # Clean up stored references
            if hasattr(self, '_reference_latents'):
                del self._reference_latents
            if hasattr(self, '_face_prompt_embeds'):
                del self._face_prompt_embeds
